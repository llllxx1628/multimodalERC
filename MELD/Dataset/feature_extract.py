...
def meld_collate_fn(batch, tokenizer):
    label_list = [
        "neutral",
        "surprise",
        "fear",
    ...
    for sample in batch:
        ...
        concat_string = build_meld_context_input(dialogue_df, target_utterance_id)
...
def train_meld_text_encoder(
    ...
    val_dataset = MELDDataset(val_df)

    def collate_wrapper(batch):
        return meld_collate_fn(batch, model.tokenizer)
    ...
    print("Starting MELD text encoder training...")
        ...
        if len(val_preds) > 0:
            ...
            if val_f1 > best_val_f1:
                ...
                torch.save(
                    ...
                    os.path.join(output_model_dir, "meld_text_encoder.pth"),
                )
        else:
            print(f"Epoch {epoch + 1}: No valid validation samples")

    print(f"MELD training completed. Best Val F1: {best_val_f1:.4f}")
    return best_val_f1
...
def extract_text_features(
    ...
    checkpoint_path = os.path.join(model_dir, "meld_text_encoder.pth")
    ...
    print(f"MELD features saved to {output_json}")
...
def extract_visual_features(
    ...
    timesformer_feature_extractor = AutoImageProcessor.from_pretrained("facebook/timesformer-base-finetuned-k400")
    ...
    for d in tqdm(sub_dirs, desc="Extracting visual features"):
        ...
        if not os.path.exists(audio_file):
            print(f"Skipping {d}, audio file not found.")
            continue

        audio, sr = librosa.load(audio_file, sr=None)
        audio_tensor = compute_audio_features(audio, sr, fixed_num_frames=fixed_num_frames).to(device)
        audio_tensor = audio_tensor.unsqueeze(0)

        face_videos = [f for f in os.listdir(sample_path) if f.endswith('.mp4')]
        if not face_videos:
            print(f"No .mp4 found in {sample_path}, skipping.")
            continue
        ...
        for face_video in face_videos:
            ...
            if not cap.isOpened():
                print(f"Unable to open {video_path}, skipping.")
                continue
            ...
            pixel_values = timesformer_feature_extractor(frames, return_tensors="pt")["pixel_values"].to(device)
        ...
        if face_scores:
            ...
            best_dist = face_scores[best_face]
            print(f"{d} => best face: {best_face}, distance: {best_dist:.4f}")

            best_frames = frames_dict[best_face]
            pixel_values = timesformer_feature_extractor(best_frames, return_tensors="pt")["pixel_values"].to(device)
            with torch.no_grad():
...
def main():
    datasets_info = [
        {
            "name": "train",
            "video_dir": "/path/to/meld/train_video",
            "audio_dir": "/path/to/train_audio",
            "text_csv": "/path/to/train_sent_emo.csv",
            "output_feature_dir": "/MELD/Dataset/Data/train_features",
        },
        {
            "name": "dev",
            "video_dir": "/path/to/dev_video",
            "audio_dir": "/path/to/dev_audio",
            "text_csv": "/path/to/dev_sent_emo.csv",
            "output_feature_dir": "/MELD/Dataset/Data/dev_features",
        },
        {
            "name": "test",
            "video_dir": "/path/to/test_video",
            "audio_dir": "/path/to/test_audio",
            "text_csv": "/path/to/test_sent_emo.csv",
            "output_feature_dir": "/MELD/Dataset/Data/test_features",
        },
    ]
    fine_tuned_model_dir = "/MELD/Dataset/Data/fine_tuned_model_meld"
    ...
    if not os.path.exists(fine_tuned_model_dir):
        print("Starting MELD fine-tuning...")
    ...
    print("MELD processing completed!")
